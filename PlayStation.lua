...
local function DestroyYep()
    for x = 1,69 do 
        if game.CoreGui:FindFirstChild("DetectedMafia") then game.CoreGui:FindFirstChild("DetectedMafia"):Destroy() end
    end
end
...
function Library:CreateWindow(windowname,windowinfo)
    local DetectedMafia = Instance.new("ScreenGui")
    ...
    DetectedMafia.Name = "DetectedMafia"
    DetectedMafia.Parent = game.CoreGui
    DetectedMafia.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    DetectedMafia.ResetOnSpawn = false
    
    Frame.Parent = DetectedMafia
